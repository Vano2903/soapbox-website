<script lang="ts">
	...
	function parseInlineTokens(content: string): InlineToken[] {
		...
		for (const match of content.matchAll(iconPattern)) {
			...
			tokens.push({ kind: 'icon', component: resolveDocsIcon(iconName, 'CircleQuestionMark') });
	...
	const docsCategoriesView = $derived(
		docsContent.categories.map((category) => ({
			...category,
			iconComponent: resolveDocsIcon(category.icon, 'CircleQuestionMark'),
	...
	function isCategoryActive(category: DocsCategory): boolean {
		return (
			activeSectionId === category.id || category.pages.some((page) => page.id === activeSectionId)
		);
	}

	onMount(() => {
		const sectionElements = Array.from(
			document.querySelectorAll<HTMLElement>('[data-doc-nav-section="true"]')
		);
...
<main class="pb-16">
	<div class="bg-base-100 min-h-screen">
		<section
			class="border-base-300 relative overflow-hidden border-b bg-linear-to-tl from-zinc-100 via-stone-100 to-red-400 shadow-[0_12px_28px_-24px_rgba(31,41,55,0.42)]"
		>
			<div
				class="pointer-events-none absolute inset-0 bg-[radial-gradient(95%_70%_at_85%_0%,rgba(220,38,38,0.30),transparent_68%),radial-gradient(70%_55%_at_18%_10%,rgba(185,28,28,0.18),transparent_72%),radial-gradient(62%_66%_at_92%_100%,rgba(236,239,243,0.80),transparent_74%),linear-gradient(145deg,rgba(247,249,251,0.46)_4%,rgba(255,255,255,0)_42%,rgba(236,239,243,0.36)_100%)]"
			></div>
			<div class="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-14">
				<div class="relative space-y-4">
					<div
						class="inline-flex items-center gap-2 rounded-full border border-red-300/80 bg-red-50 px-4 py-2 text-xs font-semibold tracking-[0.2em] text-red-700 uppercase"
					>
						<HeaderIcon class="h-4 w-4" />
						Documentazione Sito
					</div>
					<h1 class="max-w-4xl text-4xl leading-tight font-bold md:text-5xl">
						Tutta la documentazione in un posto: aggiornata e facile da navigare
					</h1>
					<p class="text-base-content/70 max-w-3xl md:text-lg">
						Tutto il contenuto del sito web è riassunto in questa sezione dedicata alla spiegazione
						della struttura delle pagine e degli aiuti contestuali disponibili.
					</p>
				</div>
			</div>
		</section>
		...
		<section
			id="faq"
			data-doc-nav-section="true"
			class="mx-auto mt-8 max-w-7xl scroll-mt-28 px-4 md:px-6"
		>
			<div class="border-base-300 bg-base-200/50 rounded-3xl border p-6 md:p-8">
				<h2 class="text-2xl font-bold md:text-3xl">Domande frequenti</h2>
				<p class="text-base-content/70 mt-2 text-sm">
					Scorciatoie rapide per raggiungere velocemente la sezione desiderata.
				</p>
				<div class="mt-6 grid gap-3 md:grid-cols-2">
					{#each docsContent.faq as item}
						<div
							class="group border-base-300 bg-base-100/95 rounded-2xl border p-4 shadow-sm transition-colors duration-250 hover:border-red-100/80 hover:bg-red-50/50"
						>
							<p class="text-sm font-medium">{item.question}</p>
							<a
								href={`#${item.targetId}`}
								class="text-primary mt-3 inline-flex items-center gap-2 text-sm font-semibold"
							>
								<span class="transition-transform duration-300 group-hover:translate-x-1.5"
									>Vedi {item.linkLabel}</span
								>
								<ArrowRight
		...
		<div class="mx-auto mt-10 max-w-7xl px-4 md:px-6">
			<div class="grid gap-8 lg:grid-cols-[18rem_minmax(0,1fr)]">
				<aside
					class="lg:border-base-300 hidden h-fit lg:sticky lg:top-24 lg:block lg:border-r lg:pr-5"
				>
					<p class="text-base-content/50 mb-4 text-xs font-semibold tracking-[0.2em] uppercase">
					...
					<nav class="space-y-2">
						...
						{#each docsCategoriesView as category}
							<div>
								...
								<div class="border-base-300 mt-1 space-y-1 border-l pl-3">
				...
				<div class="space-y-16">
					{#each docsCategoriesView as category}
						<section
							id={category.id}
							data-doc-nav-section="true"
							class="border-base-300 scroll-mt-28 border-t-4 pt-9"
						>
							<div class="bg-base-200/35 rounded-3xl p-6 md:p-7">
								<div class="flex items-start gap-4 md:items-center">
									...
									<div>
										...
										<p class="text-base-content/70 mt-2 max-w-4xl">{category.intro}</p>
									</div>
								</div>
							</div>
							...
							<div class="mt-4 space-y-4">
								{#each category.pages as page}
									<article
										id={page.id}
										data-doc-nav-section="true"
										class="border-base-300 bg-base-100 scroll-mt-28 rounded-3xl border shadow-sm"
									>
										<div class="p-6 md:p-7">
											<div class="flex flex-wrap items-center justify-between gap-3">
												<div class="flex items-center gap-2">
													<page.iconComponent class="text-primary h-5 w-5" />
													<h3 class="text-xl font-semibold md:text-2xl">{page.title}</h3>
												</div>
											...
											<p class="text-base-content/70 mt-3 max-w-3xl text-sm">{page.summary}</p>

											{#if page.notes && page.notes.length > 0}
												<div class="bg-base-200 mt-5 rounded-2xl p-4">
													<p class="text-sm font-semibold">Note</p>
													<ul class="text-base-content/70 mt-2 list-disc space-y-1 pl-5 text-sm">
														{#each page.noteTokens as noteTokens}
															<li>
																{#each noteTokens as token}
																	{#if token.kind === 'icon'}
																		<token.component
																			class="text-primary mx-1 inline h-4 w-4 align-[-3px]"
																		/>
																	{:else}
																		{token.value}
																	{/if}
											...
											{#if page.chapters.length > 0}
												<div
													class={`mt-5 grid gap-3 ${page.chapters.length === 1 ? 'md:grid-cols-1' : page.chapters.length === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'}`}
												>
													{#each page.chapters as chapter}
														<div class="bg-base-200 rounded-2xl p-4">
															<p class="text-primary text-sm font-semibold">{chapter.title}</p>
															<p class="text-base-content/70 mt-2 text-sm">{chapter.body}</p>
														</div>
													{/each}
												</div>
											{/if}
											...
											{#if page.helpKeys.length > 0}
												<div class="border-base-300 mt-5 border-t pt-5">
													...
													<div
														class={`mt-4 grid grid-cols-1 gap-4 ${page.helpKeys.length > 1 ? 'md:grid-cols-2' : ''}`}
													>
														{#each page.helpKeys as helpKey}
															{@const help = contextualHelpsView[helpKey]}
															{#if help}
																<article
																	id={help.docReference}
																	data-doc-section="true"
																	class="scroll-mt-28 rounded-2xl border border-red-100 bg-red-50/60 p-4"
																>
																	<p
																		class="text-xs font-semibold tracking-[0.2em] text-red-700/70 uppercase"
																	>
																		{help.location}
																	</p>
																	...
																	<p class="text-base-content/80 text-sm font-semibold">
																		Nel dettaglio:
																	</p>
